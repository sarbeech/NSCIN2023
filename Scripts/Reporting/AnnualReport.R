...
FN121trap <- get_FN121_Trapnet(list(prj_cd = c("LOA_IA23_NSF", "LOA_IA23_NSI", "LOA_IA23_NSK")))
FN121 <- full_join(FN121, FN121trap)
...
FN121 <- FN121 %>% separate(SITP, into = c("substrate", "cover"), 1)
FN121 <- FN121 %>% mutate(subtype = ifelse(substrate == 3 | substrate == 4, "Soft", "Hard"))
...
write.csv(siteinfo, "./Data/Exports/SiteSummary2023.csv", row.names = F)

################################################################################################################################################
######################   Code between this section is not ran yet and includes mapping #########################################################

# make maps
# identify coordinates
coordinates(FN121) <- c("LONGITUDE", "LATITUDE")
...
fishcaught <- FN123 %>%
  group_by(PRJ_CD) %>%
  dplyr::summarise(totalfish = sum(CATCNT), species = length(unique(SPC)));fishcaught
...
catch_sumbynet <- left_join(catch_sumbynet, FN121);catch_sumbynet
...
catch_sumbyspc <- FN123 %>%
  ...
  dplyr::summarize(catch = sum(CATCNT), 
                   subcnt = sum(SUBCNT), 
                   biocnt = sum(BIOCNT, na.rm = T));catch_sumbyspc
...
Avelength <- FN124 %>%
  group_by(PRJ_CD, SPC) %>%
  dplyr::summarize(meanlength = sum(SIZ * SIZCNT) / sum(SIZCNT)) %>% 
  rename(Species = SPC);Avelength

CUE <- left_join(CUE, Avelength)
write.csv(CUE, "./Data/Exports/Catchsummary2023.csv", row.names = F)
...
ggplot(filter(filteredcue, Year > 2005, EMB %in% c("Toronto Islands", "Weller's Bay", "Upper")), aes(Year, meanCUE)) +
  geom_bar(color = "black", stat = "identity", width = 0.6) +
  facet_wrap(~EMB) +
  labs(y = "CUE") +
  theme_classic() +
  theme(axis.text = element_text(size = 19, color = "black"), axis.title = element_text(size = 28))
...
Ages <- FN125 %>%
  filter(!is.na(AGE)) %>%
  filter(SPC %in% c("131", "311", "313", "314", "316", "317", "334", "331", "319")) %>%
  mutate(Yearclass = 2022 - AGE, EMB = ifelse(PRJ_CD == "LOA_IA22_NST", "Toronto Islands", "Upper Bay of Quinte")) %>%
  mutate(EMB = ifelse(PRJ_CD == "LOA_IA22_NSB", "Weller's Bay", EMB))
Ages <- append.spc.names(Ages)
Agesummary <- Ages %>%
  group_by(PRJ_CD, EMB, SPC, SPC_NM, AGE, Yearclass) %>%
  dplyr::summarise(SumAges = n())

ggplot(Agesummary, aes(as.factor(AGE), SumAges)) +
  geom_bar(stat = "identity", col = "black", fill = "black") +
  facet_grid(SPC_NM ~ EMB, switch = "y") +
  theme_classic() +
  labs(y = "Count", x = "Age") +
